...
<?= $this->section('main') ?>
  ...
  <div class="col-lg-6 mb-5">
    <div class="card rounded shadow p-3">
      <form action="<?= base_url('/dashboard/absences/update/'. $absence['id']) ?>" method="post" enctype="multipart/form-data">
        <?= csrf_field() ?>
        <div class="mb-3">
          ...
          <select name="member_id" class="form-select" id="select2">
            <?php foreach ($members as $member) : ?>
              <option value="<?= $member['id'] ?>" <?= $absence['member_id'] == $member['id'] ? 'selected' : '' ?> ><?= $member['name'] ?> | <?= $member['nrp'] ?> | <?= $member['rank'] ?></option>
            <?php endforeach ?>
          </select>
        ...
        <div class="mb-3">
          ...
          <select name="absence" class="form-select">
            <option value="Ijin" <?= $absence['absence'] == 'Ijin' ? 'selected' : '' ?>>Ijin</option>
            <option value="Masuk" <?= $absence['absence'] == 'Masuk' ? 'selected' : '' ?>>Masuk</option>
            <option value="Pulang" <?= $absence['absence'] == 'Pulang' ? 'selected' : '' ?>>Pulang</option>
          </select>
          <div class="invalid-feedback">
        ...
        <div class="mb-3">
          ...
          <input required type="date" name="date"class="form-control" value="<?= $absence['date'] ?>">
          <div class="invalid-feedback">
            <?= session('errors.date') ?>
          </div>
        </div>
        <div class="mb-3">
          ...
          <img src="<?= base_url('/image/absence/'. $absence['image']) ?>" class="d-block img-preview img-fluid mb-3 col-sm-5">
        ...
        <div class="mb-3">
          ...
          <textarea name="description" cols="10" rows="5" class="form-control"><?= $absence['description'] ?></textarea>
          <div class="invalid-feedback">
